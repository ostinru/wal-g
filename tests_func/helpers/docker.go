...
func (inf *Infra) RecreateContainers() error {
	actions := []string{"down", "--volumes", "--timeout", "0"}
	if err := inf.callCompose(actions); err != nil {
		return err
	}
...
func (inf *Infra) callCompose(actions []string) error {
	// FIXME: regression in behaviour (--log-level was ommited in some cases!)
	baseArgs := []string{"--log-level", "WARNING", "compose", "--file", inf.config, "-p", "test"}
