...
mysql_features:
	set -e
	make go_deps
	cd tests_func/ && MYSQL_VERSION=$(MYSQL_VERSION) go test -v -count=1 -timeout 20m  --tf.test=true --tf.debug=true --tf.clean=true --tf.stop=true --tf.database=mysql

clean_mysql_features:
	set -e
	cd tests_func/ && MYSQL_VERSION=$(MYSQL_VERSION) go test -v -count=1  -timeout 5m --tf.test=false --tf.debug=false --tf.clean=true --tf.stop=true --tf.database=mysql
